...
public class EventManager extends Book {
	...
	public void begin(MainMenu menu) {
		while (true) {
			...
			commands = new String[size + getPages().length + 1]; // the 3 events themselves and the page numbers and back
			...
			listener.setAction(new ActionApplier() {
				...
				public void applyInput(String input) {
					if (input.equals("back") == true) {
						menu.begin();
						return;
					}

					for (int i = 0; i < commands.length; i++) {
						if (commands[i].equals(input) == true) {

							// is event?
							for (int j = 0; j < events.length; j++) {
								if (events[j].getName().toLowerCase().equals(input) == true) {
									viewEvent(menu, events[j]);
									return;
								}
							}

							switchPage(Integer.parseInt(commands[i]) - 1);
							begin(menu);
							return;
						}
					}
